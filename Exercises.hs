...
import           Prelude   hiding (drop, length, minimum, replicate, reverse,
                            take, unzip, zipWith, (!!), (++))
...
minimum (a :* as) = go a as
  where
    go :: (Ord a) => a -> Vec n a -> a
    go a' Nil           = a'
    go a' (a'' :* as'') = go (Prelude.min a' a'') as''

minimum' :: (Ord a) => Vec (Suc n) a -> a
minimum' (a :* Nil)       = a
minimum' (a :* b :* rest) = min a $ minimum (b :* rest)
...
exExpr :: Expr Bool
exExpr = If (Leq (Int 2) (Add (Int 2) (Int 1))) (Bool False) (Leq (Int 0) (Int 1))

eval :: Expr a -> a
eval (Int n)       = n
eval (Bool b)      = b
eval (Add e1 e2)   = (eval e1) + (eval e2)
eval (Leq e1 e2)   = (eval e1) < (eval e2)
